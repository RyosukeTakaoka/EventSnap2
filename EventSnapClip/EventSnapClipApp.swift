//
 ...
 userActivity in
                     ...
                     // URLからイベントIDを抽出
                     // 想定URL: https://yourdomain.com/event/{eventID}
                     if let eventID = extractEventID(from: incomingURL) {
                         print("✅ イベントID抽出成功: \(eventID)")
                         Task {
                             await eventViewModel.joinEvent(eventID:
 eventID)
                         }
                     } else {
     ...
     private func extractEventID(from url: URL) -> String? {
         // URLパターン: https://yourdomain.com/event/{eventID}
         ...
         // クエリパラメータからの抽出も対応
         // 例: https://yourdomain.com/?eventID={eventID}
         if let urlComponents = URLComponents(url: url,
 resolvingAgainstBaseURL: false),
            let queryItems = urlComponents.queryItems,
            let eventIDItem = queryItems.first(where: { $0.name ==
 "eventID" }),
